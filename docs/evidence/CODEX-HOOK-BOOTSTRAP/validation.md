...
- bootstrap wrapperは絶対`tool_input.workdir` / `tool_input.cwd`を優先し、相対値なら絶対`event.cwd`を基準にする。解決できない場合は許可しない。委譲先hookへ同じ対象worktreeを渡す。
...
- `mise run verify` 成功。hook 74テスト、Python bootstrap 6テストを含む。
