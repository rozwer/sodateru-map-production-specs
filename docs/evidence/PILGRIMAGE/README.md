...
`mise exec -- node --experimental-transform-types --test server/plugins/pilgrimage/service.test.ts` : 4件PASS。実SQLiteの再open/保存順/停止保持/本人・モード分離/設定版競合/未確認拒否/更新/AI採用rollback/AI候補IDを確認。経路とAIはこの単体テストでは明示したtest double。
...
- 実共通AIによる生成→採用の実行証拠（固有task接続コードと単体検証は実装済み）。
