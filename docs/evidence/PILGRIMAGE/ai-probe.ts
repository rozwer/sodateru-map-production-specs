...
import { pilgrimageTask } from '../../../server/plugins/pilgrimage/ai.ts';
import { assertAiAllowed, readSettings, patchSettings } from '../../../server/features/settings/service.ts';
...
const dbs=openDatabases({...paths,migrations:[{id:'places/001',sql:readFileSync(resolve(routeRoot,'server/db/migrations/places/001-details.sql'),'utf8')},{id:'plugins/001',sql:readFileSync(resolve(pluginRoot,'server/db/migrations/plugins/001-plugins.sql'),'utf8')},{id:'settings/001',sql:readFileSync(new URL('../../../server/db/migrations/settings/001-settings.sql',import.meta.url),'utf8')},{id:'pilgrimage/001',sql:readFileSync(new URL('../../../server/db/migrations/pilgrimage/001-pilgrimage.sql',import.meta.url),'utf8')}]});
...
try {
 ...
 const preferences=readSettings(dbs.live,ctx.personId);patchSettings(dbs.live,ctx.personId,preferences.version,{ai:{...preferences.ai,enabled:true,allowLocation:true}});
 const deps={search:searchSources,pluginState:getPluginState,places:placesService,routes:createRoutesService,ai:{assertRunAdoptable:ai.assertRunAdoptable,appendAppliedRef:ai.appendAppliedRef}};
 ...
 const feature=pilgrimageFeature(db=>new PilgrimageService(db,deps),ai);
 ...
 const accepted=await call('POST','/plugins/pilgrimage/ai-proposals',{searchId:search.id,relationIds:['hida-2','hida-1'],settingsVersion:setting.version,text:'飛騨古川駅から図書館へ徒歩で巡る順序を提案してください。営業時間は不明として扱ってください。'});
 assert.equal(accepted.status,202,JSON.stringify(accepted.body));let aiRun=accepted.body.data;
 const deadline=Date.now()+180000;while(['pending','running'].includes(aiRun.status)&&Date.now()<deadline){await delay(1000);const polled=await call('GET',`/plugins/pilgrimage/ai-proposals/${aiRun.id}`);assert.equal(polled.status,200);aiRun=polled.body.data;}
 assert.equal(aiRun.status,'complete',JSON.stringify(aiRun.error));assert.equal(aiRun.model,'gpt-5.6-luna');
 const selection={ai:{runId:aiRun.id,attempt:aiRun.attempt},searchId:search.id,orderedRelationIds:aiRun.result.orderedRelationIds,mode:'walking' as const,title:'飛騨古川駅 → 飛騨市図書館',settingsVersion:setting.version,acknowledgeUnverified:false};
 ...
 assert.ok(ai.readAppliedRefs(dbs.live,ctx,aiRun.id).some((ref:any)=>ref.type==='pilgrimage-plan'&&ref.id===plan.id));
 const aiReopened=new DatabaseSync(paths.livePath);try{assert.deepEqual((await ai.getRun(aiReopened,ctx,aiRun.id)).result,aiRun.result);}finally{aiReopened.close();}
 const output={aiRun,aiAppliedRefs:ai.readAppliedRefs(dbs.live,ctx,aiRun.id),httpResponses:responses,checkedAt:new Date().toISOString(),dependencies:{routeRoot,pluginRoot,aiRoot,integrated:routeRoot===resolve('.')&&pluginRoot===resolve('.')&&aiRoot===resolve('.')},assertions:{realAi:true,realSettingsPermission:true,aiSqliteReopen:true,aiAdoptionRecorded:true,realHttp:true,authenticatedIsolation:true,postReplay:true,inputValidation:true,realOfficialSources:true,realNominatimPlaces:true,realMapboxRoute:true,orderRetained:true,sqliteReopen:true,disabledOverlayOnly:true,placesAndRoutesAndRecordsRetained:true},search,plan};
 writeFileSync(resolve(proofDirectory,'ai-result.json'),JSON.stringify(output,null,2)+'\n');console.log(JSON.stringify({result:resolve(proofDirectory,'ai-result.json'),assertions:output.assertions,distanceM:plan.route.distanceM,durationSec:plan.route.durationSec,geometryPoints:plan.route.geometry.coordinates.length}));
