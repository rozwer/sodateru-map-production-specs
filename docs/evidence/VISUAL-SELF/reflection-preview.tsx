...
import "../UI-REFLECTION/preview.css";
import { MapPreview } from "../../../src/map/MapPreview";
import { MapBridge } from "../../../src/app/map-bridge";
import { photos as samplePhotos } from "../UI-INSIGHTS/preview/fixtures";
const bridge = new MapBridge("visual-self:demo");
bridge.setCamera({longitude:136.9638,latitude:35.1668,zoom:14.5,pitch:0});
bridge.showTrack("daily-track",{segments:[{id:"sample-path",coordinates:[[136.961,35.168],[136.9638,35.1668],[136.966,35.17]]}],points:[{id:"sample-cafe",coordinates:[136.961,35.168],label:"カフェ"},{id:"sample-park",coordinates:[136.966,35.17],label:"公園"}]});
const mediaMode = new URLSearchParams(location.search).get("media");
const mediaUrl = mediaMode === "error" ? "./missing-media-fixture.png" : samplePhotos.coffee;
...
{id:"q4",month:"2024年5月",question:"読書で印象に残ったことは？",answer:"静かな席で集中して読めた",status:"answered",record:{...records[0],id:"fixture-library",photoUrl:samplePhotos.books,title:"図書館で読書",place:"千種図書館",when:"5月6日（月）11:30"}}
];
function Preview() {
  const empty = new URLSearchParams(location.search).has("empty");
  const [photos, setPhotos] = useState<PhotoDraft[]>([{id:"sample-cafe",url:mediaUrl,name:"カフェ（表示サンプル）"},{id:"sample-park",url:samplePhotos.park,name:"公園（表示サンプル）"},{id:"sample-books",url:samplePhotos.books,name:"図書館（表示サンプル）"}]);
  ...
  return (
    <>
      ...
      <main className="qa-phone">
        ...
        {page === "self-home" && (
          <SelfHomeView
            map={<MapPreview bridge={bridge} label="表示サンプルの地図" className="rf-map-preview"/>}
            miniMap={<MapPreview bridge={bridge} label="表示サンプルの場所" className="rf-mini-map"/>}
