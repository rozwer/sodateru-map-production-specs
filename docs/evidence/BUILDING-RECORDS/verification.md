...
共有HEAD04bdbceec67f76dabee5b945a1e317f8c6e5e4fa（#249包含）、5173→3002、既存demo、2026-09-15 13:46頃。先の接続拒否から運用復旧後、独立タブで次を一度実施。

1. visit-confirm?visitId=7f43f9d5-2abe-4d40-b297-42ef06759a3dで候補・版1を表示。実Mapboxプレビューと帰属を表示。
2. 「行った」→「確認を保存」：成功表示、同visit版2、現在の確認済み訪問1回・用途カフェをGET結果から表示。
3. interpretation-correction?recordId=147ae580-ae1e-40b9-953b-440cd573922cで原文/用途カフェ版1を表示。カフェを外し休憩を選び「用途を保存」。同record版2・休憩と本人原文を表示。
4. ページreload後も同record版2・休憩・原文「BUILDING-RECORDS ブラウザ確認用の原文」を再表示。
5. SQLite read-onlyでvisit confirmed/version2、record purposes_json=["休憩"]/version2、本文とvisit_id保持を照合。

従って「実ブラウザ保存全般が未確認」という上の途中状態は更新。残りは実ブラウザでの取消/場所訂正/競合・実建物色・再起動復元。これらは実API/DOMの成功と区別して#217/#222へ引継ぐ。自分の5268プレビュー停止、共有5173/3002は操作していない。
