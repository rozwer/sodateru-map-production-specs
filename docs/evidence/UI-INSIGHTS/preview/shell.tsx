...
import { DiagnosisView, EvidenceView, ReviewView } from '../../../../src/features/insights/InsightViews';
...
const screens: ScreenDefinition[] = [
  { id: 'type-diagnosis', title: 'タイプ診断', layout: { header: 'back', bottomNav: true }, component: ({ navigate }) => {
    const [period, setPeriod] = useState<Period>('week');
    return <DiagnosisView value={diagnosis} period={period} onPeriod={setPeriod} onEvidence={() => navigate('trend-evidence')} onRecord={() => {}} onReview={choice => navigate('trend-review', { choice })} onExplore={() => {}}/>;
  } },
  { id: 'trend-evidence', title: 'タイプ診断', layout: { header: 'back', bottomNav: false }, component: ({ navigate }) => <EvidenceView value={insight} onRecord={() => {}} onReview={() => navigate('trend-review')}/> },
  { id: 'trend-review', title: 'タイプ診断', layout: { header: 'back', bottomNav: false }, component: ({ navigate }) => {
  ...
  { id: 'themes', title: 'わたしの地図', layout: { header: 'back', bottomNav: false }, component: ({ navigate }) => <ThemesView themes={themes} selectedId={themes[0]!.id} onSelect={() => {}} onCreate={() => navigate('theme-edit')} onEdit={() => navigate('theme-edit')} onMap={() => {}}/> },
  { id: 'theme-edit', title: 'テーマを編集', layout: { header: 'back', bottomNav: false }, component: ({ back }) => {
