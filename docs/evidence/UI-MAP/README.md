...
| 目印 | 保存済み場所のメニュー→目印追加→名前/メモ入力→配置画面→取消で下書きが保持された。保存/削除は既存 UI が利用不可と明示するため、実保存成功とは扱わない。配置画面でズーム16→17として取消した場合、元の座標・下書きは残るがズームが約17に残ることを確認し、未解決とした。 |
...
- `object-edit-R1/F01/F02/F03/F04` と `object-place-R1/F01/F02/F03` は下書き・配置/取消の UI を確認。実 CRUD、配置確定後の再取得、既存目印削除は未接続。配置取消後のズーム復元は未解決。成功扱いしない。
