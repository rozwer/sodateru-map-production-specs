...
保存済みのレイヤー/地図スタイル/本人の飾りはMAP-CUSTOMのAPIから取得する。ブラウザの下書きやプレビューを保存正本として上書きしない。APIはPLUGINSの現在有効状態と表示希望から実効表示を返す。停止完了時は対象ownerKeyをclearして実効表示を再取得し、無効なプラグインを地図設定から再有効化しない。
保存済みルートはIDからDBを読む。
