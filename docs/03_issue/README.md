...
51件は元36件、機能別CONNECT 13件、健康の余力UI/接続2件。inlineの調整・組込みTaskはboardで別管理する。

[実行と完了](execution.md) · [提供と接続](delivery.md) · [全要件/API対応](coverage.md) · [契約補完](contract-gates.md) · [index](index.json) · [原IDの両面対応](ui-connections.json)

## AのUIと実接続

UIは指定画像・実Mapbox・画面/端末操作・状態・実shell組込みを満たして通常finishする。後続CONNECTはそのUI done後に同じfeature pathを取得し、実API・保存・再取得・失敗/取消を確認する。UI単体closeを利用者向け機能完成とは呼ばない。
...
健康3画面/入口と健康の統計・取得元面は[UI-HEALTH](issues/UI-HEALTH.md) #146 → [CONNECT-HEALTH](issues/CONNECT-HEALTH.md) #148で時間が余れば着手する。非健康の設定・記録/訪問/軌跡統計は必須に残す。
