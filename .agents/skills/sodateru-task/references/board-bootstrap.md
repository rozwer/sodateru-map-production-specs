...
board導入前の `sodateru.bootstrapMode=true` は、`origin/develop` 作成前だけ `main` の初期準備を許可する。作成後の準備変更は `origin/develop` 起点の `bootstrap/<説明>` branchへcommit・pushし、PRで `develop` に統合する。`main` / `develop` への直接pushは禁止する。board導入後はpreparation Taskの状態で例外を制御する。`mise run task:phase` で段階を確認し、通常実装は `origin/develop` 起点の専用linked worktreeを使う。
