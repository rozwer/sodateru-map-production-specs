...
export type ConditionEvaluation = { key: 'avoidMotorways' | 'departAt' | 'returnBy'; status: 'applied'; reason: string; provider: string; sourceUrl: string; fetchedAt: number };
...
export type RouteProviderEvidence = { endpoint: string; attribution: string; geometryHash: string; strategy: 'balanced' | 'shortest'; warnings: { code: number; text: string }[] };
export type RoutePreview = { sourceUrl?: string; timing?: RouteTiming; providerEvidence?: RouteProviderEvidence; requestedConditions?: Conditions; conditionEvaluations?: ConditionEvaluation[]; previewId: string; waypoints: ResolvedWaypoint[]; mode: Mode; legs: RouteLeg[]; geometry: Geometry; distanceM: number; durationSec: number; provider: 'mapbox-directions' | 'valhalla'; fetchedAt: number; expiresAt: number; retention: 'storable' | 'temporary' };
...
export interface RoadProvider { route(points: Coordinates[], mode: Mode, signal?: AbortSignal, navigation?: boolean, comparisonPick?: number, conditions?: Conditions): Promise<RoadResult>; compare(points: Coordinates[], mode: Mode, signal?: AbortSignal, conditions?: Conditions): Promise<RoadResult[]>; }
