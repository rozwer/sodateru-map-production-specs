import type { TransitEvidence } from './toei.ts';
...
export type RouteTiming = { departureAt: number; arrivalAt: number; timeZone: string; providerTimePrecisionSec: 1 | 60; locations: { waypointIndex: number; localDateTime: string; timeZone: string; utcOffset: string }[] };
...
export type RoutePreview = { transitEvidence?: TransitEvidence; segmentEvidence?: SegmentEvidence; sourceUrl?: string; timing?: RouteTiming; providerEvidence?: RouteProviderEvidence; requestedConditions?: Conditions; conditionEvaluations?: ConditionEvaluation[]; previewId: string; waypoints: ResolvedWaypoint[]; mode: Mode; legs: RouteLeg[]; geometry: Geometry; distanceM: number; durationSec: number; provider: 'mapbox-directions' | 'valhalla' | 'toei-gtfs'; fetchedAt: number; expiresAt: number; retention: 'storable' | 'temporary' };
