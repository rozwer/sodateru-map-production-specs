...
export function savedRouteDto(row: any): SavedRoute {
    ...
    return { ...(route.requestedConditions ? { requestedConditions: route.requestedConditions, conditionEvaluations: route.conditionEvaluations } : {}), id: row.id, personId: row.person_id, title: row.title, waypoints: points.map((p: any) => ({ coordinates: [p.lng, p.lat], name: p.name ?? p.placeId ?? `${p.lng},${p.lat}`, placeId: p.placeId ?? null })), mode: route.mode ?? route.legs[0].mode,
