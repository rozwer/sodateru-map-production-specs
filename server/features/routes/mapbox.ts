import { RouteFault, coordinate, samePoint, type Coordinates, type Geometry, type Mode, type RouteLeg, type RouteStep } from './types.ts';
...
export class MapboxRoadProvider {
  ...
  async route(points: Coordinates[], mode: Mode, signal?: AbortSignal, navigation = false): Promise<{ legs: RouteLeg[]; geometry: Geometry; distanceM: number; durationSec: number; provider: 'mapbox-directions'; fetchedAt: number }> {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      ...
      url.search = new URLSearchParams({ access_token: this.token, alternatives: 'false', geometries: 'geojson', overview: 'full', steps: String(navigation), language: 'ja' }).toString();
      let body: any;
      try {
        ...
        try { body = await response.json(); } catch { throw new RouteFault('OUTPUT_INVALID', '道路providerの応答形式が不正です'); }
      } catch (error) {
    ...
    return { legs, geometry: { type: 'LineString', coordinates }, distanceM: legs.reduce((n, l) => n + l.distanceM, 0), durationSec: legs.reduce((n, l) => n + l.durationSec, 0), provider: 'mapbox-directions', fetchedAt: Date.now() };
  }
}
