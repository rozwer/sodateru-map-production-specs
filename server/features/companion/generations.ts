...
export class GenerationRepository {
  ...
  create(draftId: string, expectedDraftVersion: number, provider: string) {
    ...
    requireVersion(draft.version,expectedDraftVersion);
  ...
  recordProgress(id: string, update: {jobId: string; status:'queued'|'running'; progress:number}) {
    ...
    this.db.prepare("UPDATE companion_generations SET upstream_job_id=?,status=?,progress=?,version=version+1,updated_at=? WHERE id=? AND person_id=? AND status IN ('queued','running')").run(update.jobId,update.status,Math.max(item.progress,Math.floor(update.progress)),Date.now(),id,this.personId);
    return this.get(id);
  }
  complete(id: string, resultImportId: string) {
    ...
    const r=this.db.prepare("UPDATE companion_generations SET status='succeeded',progress=100,result_import_id=?,version=version+1,updated_at=? WHERE id=? AND person_id=? AND status IN ('queued','running')").run(resultImportId,Date.now(),id,this.personId);
    if (r.changes!==1) fail('GENERATION_TERMINAL');
    return this.get(id);
  }
  fail(id: string, code: string) {
    this.get(id);
    this.db.prepare("UPDATE companion_generations SET status='failed',failure_code=?,version=version+1,updated_at=? WHERE id=? AND person_id=? AND status IN ('queued','running')").run(code,Date.now(),id,this.personId);
    return this.get(id);
  }
  cancel(id: string, expected: number) {
    const item=this.get(id);
    requireVersion(item.version,expected);
    if (!active(item.status)) return item;
    const r=this.db.prepare("UPDATE companion_generations SET status='cancelled',version=version+1,updated_at=? WHERE id=? AND person_id=? AND version=? AND status IN ('queued','running')").run(Date.now(),id,this.personId,expected);
    if (r.changes!==1) fail('VERSION_CONFLICT');
    return this.get(id);
  }
  noteCancellationFailure(id: string) {
    this.get(id);
    this.db.prepare("UPDATE companion_generations SET failure_code='REMOTE_CANCEL_FAILED',version=version+1,updated_at=? WHERE id=? AND person_id=? AND status='cancelled'").run(Date.now(),id,this.personId);
  }
