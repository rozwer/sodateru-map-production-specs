...
async function ensureAi() {
  if (!ai) {
    try {
      const module = await import('../../ai/index.ts');
      const { registerTransferAiTask } = await import('./ai-task.ts');
      registerTransferAiTask();
      ai = module;
    } catch (error) {
      if ((error as {code?:string}).code === 'ERR_MODULE_NOT_FOUND') throw new CommonError('PROVIDER_UNAVAILABLE','二案生成の共通AIを準備中です。',true,{},503);
      throw error;
    }
  }
  return ai;
...
export function createTransferDependencies(db: DatabaseSync, context: RequestContext): Dependencies {
  ...
  return {
    ...
    async startRun(plan) {
      const { createConversation, startRun, canonicalHash } = await ensureAi();
    ...
    async getRun(id) { return (await ensureAi()).getRun(db,context,id); },
    ...
    appendApplied(plan) {
      if (!ai) throw new CommonError('PROVIDER_UNAVAILABLE','共通AIを再取得してください。',true,{},503);
      const {assertRunAdoptable,appendAppliedRef,canonicalHash}=ai;
