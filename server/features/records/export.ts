...
import { mediaPath, readMedia } from '../media/content.ts';
...
/** HTTP callers stream chunks so 100 large attachments do not accumulate in memory. */
export function exportRecordHtmlChunks(db: DatabaseSync, personId: string, id: string, mediaRoot: string): Iterable<string> {
  const parts = recordDocumentParts(db, personId, id, mediaRoot);
  const [before, after] = parts.template.split('<!-- RECORDS_EMBEDDED_MEDIA -->');
  return (function* () { yield before!; yield* parts.attachments(); yield after!; })();
}
export function exportRecordHtml(db: DatabaseSync, personId: string, id: string, mediaRoot: string): string {
  return [...exportRecordHtmlChunks(db, personId, id, mediaRoot)].join('');
}

/** Standalone, readable document: escaped original text and embedded media survive deletion. */
function recordDocumentParts(db: DatabaseSync, personId: string, id: string, mediaRoot: string): { template: string; attachments: () => Generator<string> } {
  ...
  for (const item of media.filter(m => m.status === 'ready')) {
    try { const path = mediaPath(mediaRoot, item.storage_key); accessSync(path, constants.R_OK); if (statSync(path).size !== item.byte_size) throw new Error('size mismatch'); }
    catch { throw new CommonError('PROVIDER_UNAVAILABLE', '媒体が欠損しているため文書を書出せません。', true, undefined, 503); }
  }
  const renderAttachments = function* () { for (const [index, m] of media.entries()) {
    let body: string;
    ...
    yield `<figure><figcaption>媒体 ${index + 1} — ${escape(m.kind)} / ${escape(m.mime_type)} / ${m.byte_size} bytes / ID: ${escape(m.id)}</figcaption>${body}</figure>`;
  } if (!media.length) yield '<p>添付なし</p>'; };
  const attachments = '<!-- RECORDS_EMBEDDED_MEDIA -->';
  const memoDetails = record.memo ? `<section><h2>メモ情報</h2><dl><dt>名前</dt><dd>${escape(record.memo.name)}</dd><dt>キーワード</dt><dd>${escape(record.memo.keywords.join('、'))}</dd><dt>由来</dt><dd>${record.memo.originRefs.map((ref: Row) => escape(`${ref.type}: ${ref.id}（版${ref.version}）`)).join('<br>')}</dd></dl></section>` : '';
