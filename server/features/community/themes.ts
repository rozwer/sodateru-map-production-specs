...
import { identity, page, type SocialContext } from '../friends/service.ts';
...
export function patchThemeSharing(db: DatabaseSync, personId: string, id: string, version: number, input: { visibility: string; sharedWith: string[] }) {
  ...
  if (!['private', 'public', 'selected'].includes(input.visibility) || !Array.isArray(input.sharedWith) || input.sharedWith.length > 100) throw invalid('共有設定が不正です');
  ...
  db.prepare(`INSERT INTO community_theme_shares(id,visibility,shared_with_json,version,created_at,updated_at)
...
export function getSharedTheme(db: DatabaseSync, context: SocialContext, id: string, reads: CommunityReads) {
  ...
  const records: any[] = [];
  for (const recordId of JSON.parse(String(theme.record_ids_json)) as string[]) {
