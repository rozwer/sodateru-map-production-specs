...
export function getPlaceMetadata(db:DatabaseSync,placeId:string) {
  const row=db.prepare("SELECT opening_hours_json,entrances_json,corrections_json FROM place_details WHERE place_id=?").get(placeId) as any;
  const corrections=JSON.parse(row?.corrections_json??"{}");
  return {openingHours:JSON.parse(row?.opening_hours_json??"null") as OpeningHours|null,entrances:JSON.parse(row?.entrances_json??"[]") as Entrance[],
