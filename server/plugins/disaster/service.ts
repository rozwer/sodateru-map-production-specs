...
export class DisasterService {
  constructor(readonly store: DisasterStore, readonly provider=new DisasterProvider(), readonly now:()=>number=Date.now) {}
  private state() {
    const state=getPluginState(this.store.db,this.store.context);
  ...
  read() {
    ...
    return {
      ...
      map:{action:canApply ? 'apply' as const : 'clear' as const,ownerKey:plugin?.ownerKey ?? (result ? `plugin:${result.installId}` : null),
  ...
  async refresh(expected: number) {
    ...
    return transaction(this.store.db,()=>{
      ...
      const snapshot: Snapshot={resultId:randomUUID(),dataKind:'live',settings,installId:before.setting!.installId,
        ...
        expiresAt:this.now()+(settings.layerIds.includes('rainfall') ? 10*60_000 : 24*60*60_000),status,layers,
