...
try {
  ...
  const candidates=success(await api("/bike/place-candidates","POST",{searchId:search.id},undefined,candidateKey)).data;
  assert.equal(candidates.settingsVersion,search.settingsVersion);assert.equal(candidates.settingsHash,search.settingsHash);
  assert.equal(candidates.candidates.items.length,search.places.length);
  assert.deepEqual(success(await api("/bike/place-candidates","POST",{searchId:search.id},undefined,candidateKey)).data,candidates);
  const candidate=candidates.candidates.items[0];
  const original=search.places.find((p:any)=>p.id===candidate.candidateId);
  assert.equal(candidate.provider,"nominatim");assert.equal(candidate.sourceUrl,original.source.url);
  assert.equal(candidate.fetchedAt,original.source.fetchedAt);assert.equal(candidate.retention,"storable");
  const adoptedPlace=success(await api("/places","POST",{id:randomUUID(),mode:"candidate",resultId:candidates.candidates.resultId,candidateId:candidate.candidateId})).data;
  assert.equal(adoptedPlace.sourceUrl,candidate.sourceUrl);assert.equal(adoptedPlace.fetchedAt,candidate.fetchedAt);
  ...
  const savedPlace=success(await api(`/places/${adoptedPlace.id}`)).data.place;
  assert.equal(savedPlace.id,adoptedPlace.id);assert.equal(savedPlace.sourceUrl,adoptedPlace.sourceUrl);assert.equal(savedPlace.fetchedAt,adoptedPlace.fetchedAt);
  assert.equal((await api("/bike/place-candidates","POST",{searchId:search.id},undefined,candidateKey)).status,410);
  const refreshedCandidates=success(await api("/bike/place-candidates","POST",{searchId:search.id})).data;
  assert.notEqual(refreshedCandidates.candidates.resultId,candidates.candidates.resultId);
  const reusedPlace=success(await api("/places","POST",{id:randomUUID(),mode:"candidate",resultId:refreshedCandidates.candidates.resultId,candidateId:candidate.candidateId})).data;
  assert.equal(reusedPlace.id,adoptedPlace.id,"same OSM object must not become a duplicate place");
  ...
  assert.equal((await api("/bike/place-candidates","POST",{searchId:search.id})).status,409);
