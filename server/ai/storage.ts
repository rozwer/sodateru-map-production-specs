...
export function createConversation(db:DatabaseSync,ctx:AiContext,input:{id:string;purpose:string;title:string;recordId:string|null}) {
 if(!input||typeof input.id!=='string'||!input.id.trim()||input.id.length>80||!['consult','reflection','analysis','comparison'].includes(input.purpose)||typeof input.title!=='string'||!input.title.trim()||input.title.length>200||!(input.recordId===null||typeof input.recordId==='string'))throw aiError('INVALID_INPUT','会話入力が不正です');
...
export function patchConversation(db:DatabaseSync,ctx:AiContext,id:string,title:string,version:number) {
 if(typeof title!=='string'||!title.trim()||title.length>200)throw aiError('INVALID_INPUT','題名が不正です');
