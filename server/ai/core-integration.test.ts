...
import { configureAi, registerAiTask, appendAppliedRef } from './index.ts';
...
test('CORE HTTP receipts, SQLite persistence, cancellation replay and retry survive restart',async()=>{
 ...
 try{
  ...
  const adopted={type:'map-settings' as const,id:'map-http',version:2,contentHash:'a'.repeat(64)};
  appendAppliedRef(databases.live,{personId:session.body.data.person.id,dataMode:'live',requestId:randomUUID(),signal:new AbortController().signal},'a-http',adopted);
  const completed=await request('/messages/a-http');
  assert.deepEqual(completed.body.data.appliedRefs,[adopted]);assert.equal(completed.body.data.output.value.explanation,'現在の水辺');
  ...
  const restored=await request('/messages/a-http');assert.deepEqual(restored.body.data.appliedRefs,[adopted]);assert.equal(restored.body.data.output.value.explanation,'現在の水辺');
