...
export function attachGrowth(map: mapboxgl.Map, getItems: () => GrowthDisplay[], onSelect: (group: BuildingGrowth) => void, onBuildings: (buildings: Building[]) => void, interactive: boolean) {
  ...
  const update = () => {
    ...
    for (const feature of features) {
      ...
      if (!old || JSON.stringify(candidate.geometry).length > JSON.stringify(old.geometry).length) catalog.set(key, candidate);
    }
    ...
    for (const group of groups) {
      ...
      const element = document.createElement('button'); element.type = 'button'; element.className = 'map-growth-badge'; element.dataset.stage = String(group.stage); element.dataset.buildingKey = group.building.key;
