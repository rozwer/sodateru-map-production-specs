...
export function MapScene(props: Props) {
  ...
  useEffect(() => {
    ...
    map.on('style.load', () => {
      ...
      map.addLayer({ id: 'sodateru-growth-building', type: 'fill-extrusion', source: 'sodateru-growth', slot: 'top', paint: { 'fill-extrusion-color': ['get', 'color'], 'fill-extrusion-base': ['get', 'base'], 'fill-extrusion-height': ['+', ['get', 'height'], 0.06], 'fill-extrusion-opacity': ['get', 'opacity'], 'fill-extrusion-color-transition': { duration: 420 }, 'fill-extrusion-opacity-transition': { duration: 420 } } });
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('sodateru:play-growth', play);
  }, []);
