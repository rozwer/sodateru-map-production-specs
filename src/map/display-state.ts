...
import type { Building } from './growth-rules';
...
type DisplayState = { placement: Pick<ObjectPreviewProps, 'color' | 'size'> | null; decorations: MapDecoration[]; growth: GrowthItem[]; buildings: Building[] };
class DisplayStore {
  private state: DisplayState = { placement: null, decorations: [], growth: [], buildings: [] };
  private listeners = new Set<() => void>();
  constructor(bridge: MapBridge) {
    let scope = bridge.getSnapshot().scopeKey;
    bridge.subscribe(() => { if (scope !== bridge.getSnapshot().scopeKey) { scope = bridge.getSnapshot().scopeKey; this.state = { placement: null, decorations: [], growth: [], buildings: [] }; this.listeners.forEach(listener => listener()); } });
  }
