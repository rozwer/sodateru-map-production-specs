...
import type { MapCamera } from '../../app/map-bridge';
...
function MapScreen({ route, navigate, back, scopeKey, active = true }: Props) {
  ...
  const [ui, setUi] = useScreenState({ tab: 'results' as 'results' | 'place', more: false, resultsCamera: null as MapCamera | null });
  ...
  useEffect(() => {
    if (!active) return;
    const candidate = bridge.onSelect('map-search', selection => { if (selection.kind === 'candidate') { setUi(value => ({ ...value, tab: 'place', resultsCamera: value.tab === 'place' ? value.resultsCamera : bridge.getSnapshot().camera })); session.selectCandidate(selection.id, bridge); } });
  ...
  const savedPlaceRoute = !!route.params.placeId;
  const candidate = savedPlaceRoute ? null : state.result?.items.find(item => item.candidateId === state.selectedCandidateId);
  const place = state.detail ? detailPresentation(state.detail) : candidate ? candidatePresentation(candidate) : null;
  const expired = !savedPlaceRoute && !!state.result && state.result.expiresAt <= now;
  ...
  const selected = (id: string) => { setUi(value => ({ ...value, tab: 'place', resultsCamera: value.tab === 'place' ? value.resultsCamera : bridge.getSnapshot().camera })); session.selectCandidate(id, bridge); };
  const showResults = () => { if (ui.resultsCamera) { bridge.setCamera(ui.resultsCamera); bridge.focus('map-search', { center: [ui.resultsCamera.longitude, ui.resultsCamera.latitude], zoom: ui.resultsCamera.zoom }); } setUi(value => ({ ...value, tab: 'results', more: false })); };
  const nearby = () => { void session.search(bridge, 'coffee'); setUi(value => ({ ...value, tab: 'results' })); };
  const nearbySelected = (id: string) => { setUi(value => ({ ...value, tab: 'place', resultsCamera: bridge.getSnapshot().camera })); session.selectNearby(id, bridge); };
  return <div className="map-feature" data-testid="map-screen">
    ...
    {!savedPlaceRoute && (state.result || state.loading) && <div className="map-result-tabs" role="tablist" aria-label="検索結果の表示"><button type="button" role="tab" aria-selected={ui.tab === 'results'} onClick={showResults}>{m.searchResults}{state.result ? ` ${state.result.items.length}件` : ''}</button><button type="button" role="tab" aria-selected={ui.tab === 'place'} disabled={!place} onClick={() => setUi(value => ({ ...value, tab: 'place' }))}>{m.placeInfo}</button></div>}
    ...
    {savedPlaceRoute && !place && <button type="button" className="map-detail-back" onClick={back}><span aria-hidden="true">←</span>地図へ戻る</button>}
    {place && ui.tab === 'place' && <><button type="button" className="map-detail-back" onClick={() => savedPlaceRoute ? back() : showResults()}><span aria-hidden="true">←</span>{state.result && !savedPlaceRoute ? '検索結果へ戻る' : '地図へ戻る'}</button><PlaceDetailPanel place={place} variant={savedPlaceRoute ? 'place' : state.result ? 'search' : 'place'} saved={!!state.selectedPlaceId} saving={state.saving} canSave={!expired && candidate?.retention !== 'temporary'} onSave={() => void save()} onRoute={routeTo} onShare={() => navigate('sharing', state.selectedPlaceId ? { placeId: state.selectedPlaceId } : {})} onMore={() => setUi(value => ({ ...value, more: !value.more }))} />
    ...
    {!savedPlaceRoute && !state.result && !place && !state.loading && !state.detailLoading && !state.error && !state.detailError && <><div className="map-section-heading"><h2>{m.area}</h2><p className="map-muted map-small">暮らしやスポットを見つけて、あなたの地図を育てていきましょう。</p></div><button type="button" className="map-area-card" onClick={nearby}><Icon name="pin"/><div><h3>{m.placeInfo}</h3><p>地図を動かして、気になる場所を選んでください。</p></div><span aria-hidden="true">›</span></button>{state.placesLoading && <Status kind="loading">保存した場所を読み込み中…</Status>}{!state.placesLoading && state.places.length === 0 && <Status>保存した場所はまだありません。検索して気になる場所を保存できます。</Status>}<button type="button" className="map-outline map-wide" onClick={() => navigate('map-layers')}>表示レイヤーを確認</button>{state.places.length > 0 && <section><h3>保存した場所</h3><ul className="map-saved-list">{state.places.map(item => <li key={item.id}><button type="button" onClick={() => navigate('map', { placeId: item.id })}><MapIcon name="bookmark"/><span>{item.name}</span>›</button></li>)}</ul>{state.nextPlaceCursor && <button className="map-outline" type="button" onClick={() => void session.loadPlaces(bridge, true)}>さらに表示</button>}</section>}</>}
  </div>;
}
...
function LayersScreen({ back, active = true }: Props) {
  const bridge = useMapBridge();
  const [values, setValues] = useScreenState({ themes: true, suggestions: true, friends: false, motorcycle: false });
  useEffect(() => { if (!active) return; return () => mapDisplay(bridge).setLayerPreview(null); }, [active, bridge]);
  useEffect(() => { if (active) mapDisplay(bridge).setLayerPreview({ themes: values.themes, suggestions: values.suggestions, friends: values.friends }); }, [active, bridge, values]);
  ...
  ] as const).map(([key, title, help, icon]) => <label className="map-layer-row" key={key}><span className="map-layer-icon">{icon}</span><span><strong>{title}</strong><p>{help}{key === 'motorcycle' ? '（表示データ未接続）' : ''}</p></span><input type="checkbox" role="switch" checked={values[key]} disabled={key === 'motorcycle'} onChange={event => setValues(previous => ({ ...previous, [key]: event.target.checked }))} aria-label={title}/></label>)}</div><Status kind="unavailable">切替はこの画面の地図プレビューです。設定の保存とバイク専用表示は現在利用できません。</Status><button type="button" className="map-primary map-wide" onClick={back}>{m.backMap}</button></div>;
}

const drafts = new Map<string, DecorationDraft>();
const placementCameras = new Map<string, MapCamera>();
function ObjectEditScreen({ route, navigate, back, scopeKey, active = true }: Props) {
  const bridge = useMapBridge(); const key = `${scopeKey}:${route.params.objectId || 'new'}`;
  const camera = bridge.getSnapshot().camera;
  const origin: [number, number] = [route.params.longitude !== undefined && Number.isFinite(Number(route.params.longitude)) ? Number(route.params.longitude) : camera.longitude, route.params.latitude !== undefined && Number.isFinite(Number(route.params.latitude)) ? Number(route.params.latitude) : camera.latitude];
  const [draft, setDraft] = useScreenState<DecorationDraft>(() => drafts.get(key) || { name: '', memo: '', color: m.colors[0].value, size: 'medium', coordinates: origin });
  ...
  useEffect(() => { if (active) { const placed = drafts.get(key); if (placed) setDraft(placed); } }, [active, key]);
  ...
  return <div className="map-feature"><ObjectEditPanel draft={draft} onChange={update} onRelocate={() => { drafts.set(key, draft); placementCameras.set(key, { ...bridge.getSnapshot().camera }); bridge.focus('map-objects', { center: draft.coordinates, zoom: 16 }); navigate('object-place', { objectId: route.params.objectId || 'new' }); }} onCancel={() => { drafts.delete(key); setDraft({ name: '', memo: '', color: m.colors[0].value, size: 'medium', coordinates: origin }); back(); }} onSave={() => setError('目印の保存は現在利用できません。入力は保持されています。')} onDelete={route.params.objectId ? () => setError('目印の削除は現在利用できません。') : undefined} saving={false} error={error}/></div>;
}
function ObjectPlaceScreen({ route, navigate, back, scopeKey, active = true }: Props) {
  ...
  useEffect(() => {
    ...
    const key = [scopeKey, route.params.objectId || 'new'].join(':');
    mapDisplay(bridge).setPlacement({ color: draft?.color || m.colors[0].value, size: draft?.size || 'medium' });
    return () => { mapDisplay(bridge).setPlacement(null); const previous = placementCameras.get(key); if (previous) { placementCameras.delete(key); bridge.setCamera(previous); bridge.focus('map-objects', { center: [previous.longitude, previous.latitude], zoom: previous.zoom }); } };
  }, [active, scopeKey, route.params.objectId, bridge]);
  return <div className="map-feature"><div className="map-placement-address"><Icon name="pin"/><div><strong>選択している場所</strong><p>{snapshot.camera.latitude.toFixed(5)}, {snapshot.camera.longitude.toFixed(5)}</p></div></div><div className="map-two-actions"><button type="button" className="map-secondary" onClick={back}>{m.cancel}</button><button type="button" className="map-primary" onClick={() => { const objectId = route.params.objectId || 'new'; const key = `${scopeKey}:${objectId}`; const draft = drafts.get(key); if (draft) drafts.set(key, { ...draft, coordinates: [snapshot.camera.longitude, snapshot.camera.latitude] }); placementCameras.delete(key); if (draft) back(); else navigate('object-edit', { ...(objectId === 'new' ? {} : { objectId }), placedLongitude: String(snapshot.camera.longitude), placedLatitude: String(snapshot.camera.latitude) }); }}>{m.placeHere}</button></div></div>;
}
