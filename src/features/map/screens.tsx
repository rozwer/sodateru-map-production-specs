...
function PersonalMapScreen({ route, navigate, scopeKey, active = true }: Props) {
  ...
  return <div className="map-feature map-personal-panel">
    ...
    {state.detailLoading && <Status kind="loading">場所の情報を読み込み中…</Status>}
    {state.detailError && <Status kind="error" onRetry={session.retryDetail}>{state.detailError}</Status>}
    {detail ? <><div className="map-personal-detail"><PlacePhoto place={{ ...detailPresentation(detail), photoUrl: detailedRecord?.media.find(media => media.kind === 'photo' && media.status === 'ready')?.contentUrl }}/><div><h2 className="map-icon-line"><MapIcon name="cup"/>{detail.place.name}</h2><button type="button" className="map-address-link" onClick={() => navigate('map', { placeId: detail.place.id })}><Icon name="pin"/>{detail.place.address || m.addressMissing}</button><p className="map-muted">{record?.impression}</p><blockquote>{record?.body || (state.themeId ? 'このテーマに、この場所の記録はありません。' : 'この場所の記録を選んでください。')}</blockquote></div></div>
