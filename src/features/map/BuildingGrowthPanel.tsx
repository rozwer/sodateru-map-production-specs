...
export function BuildingGrowthPanel({ buildingKey, buildings, growth, growthLoaded, places, place, onSaved, openPlace, openRecord, reloadMap }: Props) {
  ...
  return <section className="map-building-growth" aria-label="建物と体験">
    {buildingKey && <><PersonalBuildingInsight title={related[0]?.name || place?.name || 'この場所'} purposes={group?.purposes || []}/><h3>あなたの訪問と記録</h3><p>{group ? `確認済み訪問 ${group.count}回・段階${group.stage}` : growthLoaded ? '確認済みの訪問はありません' : '訪問の確認状態をまだ取得できていません'}</p>
