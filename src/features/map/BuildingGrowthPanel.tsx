...
export function BuildingGrowthPanel({ buildingKey, buildings, growth, growthLoaded, places, place, onSaved, openPlace, openRecord, reloadMap }: Props) {
  ...
  return <section className="map-building-growth" aria-label="建物と体験">
    {buildingKey && <><h2>この建物の体験</h2><p>{group ? `確認済み訪問 ${group.count}回・段階${group.stage}` : growthLoaded ? '確認済みの訪問はありません' : '訪問の確認状態をまだ取得できていません'}</p>
      ...
      {selectedBuilding && !place && <form onSubmit={event => { event.preventDefault(); void createPlace(); }}><label>この建物の場所の名前<input aria-label="この建物の場所の名前" value={name} maxLength={200} onChange={event => setName(event.target.value)}/></label><button className="map-outline" type="submit" disabled={busy || !name.trim()}>場所として保存</button><p className="map-muted">場所の保存だけでは訪問済みになりません。</p></form>}</>}
