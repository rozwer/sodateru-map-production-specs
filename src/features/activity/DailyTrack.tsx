...
export function DailyTrack({ date, onDate, entries, expandedId, revealEntry, onExpand, onEdit, onReflect, onVisit, onBack, onMenu, onRecord, map, onMapClickCapture, loading, error, onRetry, onMore, confirmedPlaces, duration, missingTrack, calendar = false, onCalendar, recordedDates, onMonth, onFitTrack, trackTimes }: {
 ...
 return <section className={`records-screen activity-daily ${calendar ? 'activity-calendar-screen' : ''}`}>
   ...
   <div className="activity-track-sheet">
     ...
     {calendar && <div className="activity-records-heading"><h3>{formatDay(date)}の記録</h3>{(!loading && !error || entries.length>0) && <span>{entries.filter(item=>item.recordId).length}件</span>}</div>}
     {!calendar && !loading && !error && missingTrack && <p className="activity-missing-track"><RecordIcon name="info"/>立ち寄った場所を表示しています。移動経路は未記録です。</p>}
     {!loading && !error && entries.length===0 && <div className="activity-empty"><p>{m.noRecords}</p><button type="button" className="records-outline" onClick={onRecord}>{m.newRecord}</button></div>}
