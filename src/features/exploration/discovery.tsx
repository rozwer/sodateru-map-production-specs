...
export function DiscoveryView({ mode, kind, targetId, feature, targets, card, saved, busy, error, notice, onKind, onTarget, onFeature, onOpen, onSaved, onExplore, onReopen, onReaction, onRetry }: {
  ...
  onOpen: () => void; onSaved: () => void; onExplore?: () => void; onReopen: (id: string) => void; onReaction: (reaction: DiscoveryReaction) => void; onRetry: () => void;
}) {
  ...
  return <section className="discovery-panel" aria-label={mode === 'saved' ? m.savedTitle : m.title}><header><span>{m.eyebrow}</span><h2>{mode === 'saved' ? m.savedTitle : m.title}</h2><p>{mode === 'saved' ? m.savedDescription : m.description}</p></header>
    ...
    {mode === 'saved' && <div className="discovery-saved">{onExplore && <button type="button" onClick={onExplore}>新しい見方を探す</button>}{!saved.length && !busy && <p>{m.noSaved}</p>}{saved.map(item => <button type="button" key={item.id} onClick={() => onReopen(item.id)}><strong>{item.bridge}</strong><small>{new Date(item.createdAt).toLocaleDateString('ja-JP')}</small></button>)}</div>}
