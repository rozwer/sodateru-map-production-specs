...
function Photo({photo}: {photo?:CandidateView['photos'][number]}) {
 const [failed,setFailed]=useState(false);
 return photo&&!failed?<img src={photo.url} alt={photo.alt} onError={()=>setFailed(true)}/>:<div className="sg-photo-missing"><Icon name="cup"/><span>{m.photoMissing}</span></div>;
}
...
export function SuggestionDetailView({item,busy,notice,onSelect,onLater,onDismiss,onBookmark,onShare,onMemo,onStop,onRoute,onVisit,children,selectDisabled,selectReason,showTools=true,memoEditor}: {
 ...
 const [photoIndex,setPhotoIndex]=useState(0);const [editing,setEditing]=useState(false);const [memo,setMemo]=useState(item.memo);
 return <section className="sg-screen sg-detail" aria-busy={busy}>
  ...
  <div className="sg-hero"><Photo key={item.photos[photoIndex]?.url} photo={item.photos[photoIndex]}/>{item.photos.length>1&&<div className="sg-gallery-controls"><button type="button" aria-label="前の写真" onClick={()=>setPhotoIndex((photoIndex+item.photos.length-1)%item.photos.length)}>‹</button><span>{photoIndex+1} / {item.photos.length}</span><button type="button" aria-label="次の写真" onClick={()=>setPhotoIndex((photoIndex+1)%item.photos.length)}>›</button></div>}</div>
  <div className="sg-detail-content"><div className="sg-title-row"><h2>{item.title}</h2><span className="sg-evaluation"><Icon name="star"/>{item.evaluation}</span></div><Tags tags={item.tags}/>
