...
export function RouteResultsPage({ draft, candidates, selectedId, onSelect, onBack, onChangeConditions, onAdopt, map, state = 'idle', notice, demo }: RouteResultsPageProps) {
  ...
  return <article className="routes-page routes-results" data-testid="route-results">
    ...
    {selected && !selected.adoptable && <p className="routes-adoption-reason" id={`${groupId}-adoption-reason`}>{selected.adoptionReason || m.cannotAdopt}</p>}
    <footer className="routes-footer routes-footer-split"><button type="button" className="routes-secondary" onClick={onChangeConditions} disabled={busy}>{m.changeConditions}</button><button type="button" className="routes-primary" disabled={busy || !selected?.adoptable} aria-describedby={selected && !selected.adoptable ? `${groupId}-adoption-reason` : undefined} onClick={() => { if (selected?.adoptable) onAdopt(selected.id); }} data-testid="route-results--select">{state === 'saving' ? m.saving : m.adopt}</button></footer>
  </article>;
}
