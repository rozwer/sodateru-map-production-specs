    ...
    <fieldset className="records-body records-composer-fields" disabled={busy}>
      ...
      {!confirmation && <>
        ...
        {editing && <>
          ...
          <section className="records-review-card"><div className="records-review-heading"><RecordIcon name="lock" /><strong>{m.visibility}</strong><button type="button" className="records-help-button" aria-label="公開範囲の説明" onClick={() => setHelp(help === 'sharing' ? null : 'sharing')}><RecordIcon name="help" /></button></div><label className="records-visibility"><RecordIcon name={draft.visibility === 'private' ? 'lock' : draft.visibility === 'public' ? 'globe' : 'people'} /><select aria-label={m.visibility} value={draft.visibility} onChange={event => setDraft({...draft,visibility:event.target.value as RecordDraft['visibility'],sharedWith:[]})}><option value="private">{m.private}</option><option value="selected">{m.selected}</option><option value="public">{m.public}</option></select></label>{draft.visibility === 'selected' && sharingControl}{help === 'sharing' && <RecordNotice>選んだ範囲に写真と本文を共有します。訪問履歴や分析結果を自動で共有しません。</RecordNotice>}</section>
        </>}
        {!valid && <p className="records-draft-state">{m.noContent}</p>}
        <button type="button" disabled={busy || !valid || (editing && draft.visibility === 'selected' && draft.sharedWith.length === 0)} className="records-primary" onClick={editing ? onSave : () => changeStep('confirmation')}>{busy ? m.saving : editing ? m.saveChanges : m.confirm}{!editing && <RecordIcon name="next" />}</button>
