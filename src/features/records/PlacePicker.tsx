...
export function PlacePicker(props: PlacePickerProps) {
  const { mode,onMode,query,onQuery,onSearch,searchSubmitted,items,selected,onSelect,onUse,onBack,onWithoutPlace,onLocate,map,busy,error,onMore } = props;
  return <section className="records-screen records-place-picker"><RecordHeading title={m.placeTitle} onBack={onBack} /><p className="records-lead">{m.placeLead}</p>
    ...
    {mode !== 'history' && <form className="records-search-form" onSubmit={event => {event.preventDefault();onSearch();}}><RecordIcon name="search" /><input aria-label="場所検索" placeholder="場所を検索" value={query} onChange={event => onQuery(event.target.value)} /><button type="button" aria-label={m.clearSearch} onClick={() => onQuery('')}><RecordIcon name="close" /></button><button type="submit" className="records-search-submit" disabled={busy || (mode === 'search' && !query.trim())}>{m.search}</button></form>}
    ...
    <div className="records-picker-results">
      ...
      {!busy && !error && items.length === 0 && <p className="records-empty">{mode === 'history' ? m.noHistory : mode === 'search' && !query.trim() ? '場所の名前を入力して検索してください。' : mode === 'search' && !searchSubmitted ? '検索を押して候補を表示してください。' : m.noCandidates}</p>}
