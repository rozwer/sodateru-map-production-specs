...
export function RecordCorrection({ body, date, interpretation, purposes, onPurposes, reason, onReason, onSavePurpose, onSaveReason, onBack, onReload, error, notice, busy }: {
  ...
  onBack: () => void; onReload?: () => void; error?: string; notice?: string; busy?: boolean;
}) {
  ...
  return <section className="records-screen records-correction"><RecordHeading title={m.correctionTitle} onBack={onBack} /><div className="records-body">
    ...
    <section className="records-correction-field"><div className="records-label-icon"><RecordIcon name="tag" /><strong>{m.purpose}</strong><button type="button" className="records-purpose-summary" aria-expanded={purposeOpen} onClick={() => setPurposeOpen(!purposeOpen)}>{purposes.join('・') || '未指定'}<RecordIcon name="next" /></button></div>{purposeOpen && <PurposesInput value={purposes} onChange={onPurposes} disabled={busy} />}<button type="button" className="records-outline" onClick={onSavePurpose} disabled={busy}>{busy ? m.saving : m.savePurpose}</button></section>
    <section className="records-correction-field"><div className="records-correction-reason"><RecordIcon name="document"/><TextField label={m.reason} value={reason} onChange={onReason} maxLength={10000} compact /></div><button type="button" className="records-outline" onClick={onSaveReason} disabled={busy}>{busy ? m.saving : m.saveReason}</button></section>
