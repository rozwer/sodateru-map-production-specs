...
def create_worktree(root, task, dest, branch, actor, paths=(), definition=None, definition_json=None, unit=None):
    ...
    if definition_json is not None:options+=['--definition-json',definition_json]
...
def main():
    ...
    command.add_argument('--definition-json', help='JSON definition registered with the first atomic claim')
    ...
    preview=tc.load_claim_definition(args.definition,args.definition_json)
    if preview is not None:
        ...
        if args.task!=preview['id']:raise tc.BoardError('Positional Task ID and definition differ')
        policy=op.policy_for(board)
    ...
    with tc.local_lock('start-'+args.task,timeout=1):
        ...
        result = create_worktree(root, args.task, dest, branch, args.actor or op.owner_at(),args.path,args.definition,args.definition_json,args.unit)
